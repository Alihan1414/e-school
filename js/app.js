...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initEvents() {
    ...
    // 7. DIRECT B2B PURCHASE PANEL NAVIGATION
    const btnOpenPurchase = document.getElementById("btn-open-b2b-purchase-portal");
    const btnBackPurchase = document.getElementById("btn-back-from-purchase");
    ...
    if (btnOpenPurchase) {
      btnOpenPurchase.addEventListener("click", () => {
        ...
        document.getElementById("view-b2b-purchase-portal").style.display = "block";
        window.scrollTo({ top: 0, behavior: "smooth" });
      });
    }

    if (btnBackPurchase) {
      btnBackPurchase.addEventListener("click", () => {
        window.HardwareManager.vibrate(25);
    ...
    // B2B Order and Cancellation
    document.querySelectorAll(".btn-select-b2b-package").forEach(btn => {
      btn.addEventListener("click", async () => {
        window.HardwareManager.vibrate(40);
        const tier = btn.dataset.tier;
        const tierName = btn.dataset.tiername;
        const price = Number(btn.dataset.price);
        const setupFee = Number(btn.dataset.setup);

        const schoolName = document.getElementById("b2b-school-name") ? document.getElementById("b2b-school-name").value : "Lycée d'Excellence Dakar";
        const contactPerson = document.getElementById("b2b-rep-name") ? document.getElementById("b2b-rep-name").value : "M. Ousmane Diop";
        const contactPhone = document.getElementById("b2b-rep-phone") ? document.getElementById("b2b-rep-phone").value : "+221 77 123 4567";
        const contactEmail = document.getElementById("b2b-rep-email") ? document.getElementById("b2b-rep-email").value : "direction@ecole.sn";
        const studentCount = document.getElementById("calc-students-count") ? Number(document.getElementById("calc-students-count").value) : 500;

        const monthlyInvoice = studentCount * price;
        const yearlyInvoice = monthlyInvoice * 12;

        currentSelectedPackage = { tier, tierName, price, setupFee, studentCount, monthlyInvoice, yearlyInvoice, schoolName };

        try {
          const resp = await fetch('/api/b2b/purchase-order', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              schoolName,
          ...
          if (data && data.orderId) {
            document.getElementById("contract-order-ref").innerText = data.orderId;
          }
        } catch (e) {
          console.warn("Order dispatch error:", e);
        }

        document.getElementById("contract-badge-type").innerText = tier.toUpperCase();
        document.getElementById("contract-school-name").innerText = schoolName;
        document.getElementById("contract-tier-name").innerText = `${tierName} ($${price}/Student/Month)`;
        document.getElementById("contract-students-count").innerText = `${studentCount} Students`;
        document.getElementById("contract-monthly-invoice").innerText = `$${monthlyInvoice.toLocaleString()} USD / Month`;
        document.getElementById("contract-yearly-invoice").innerText = `$${yearlyInvoice.toLocaleString()} USD / Year`;
        document.getElementById("contract-setup-fee").innerText = `$${setupFee} USD (Authorized Service Dispatch)`;

        const contractModal = document.getElementById("modal-official-contract");
        if (contractModal) contractModal.style.display = "flex";

        window.HardwareManager.sendLocalNotification(
          "E-School Support Center",
          `Installation Notified: ${tierName} selected ($${setupFee} setup fee). Live dispatch transmitted to alihandivanli8@gmail.com & +90 541 520 84 14.`
        );
      });
    });
    ...
    if (btnCloseContract) {
      btnCloseContract.addEventListener("click", () => {
        window.HardwareManager.vibrate(30);
        document.getElementById("modal-official-contract").style.display = "none";
        document.getElementById("view-b2b-purchase-portal").style.display = "none";
        document.getElementById("view-auth-portal").style.display = "block";
        showToast("Contract confirmed! Notice sent to technical center.");
        window.scrollTo({ top: 0, behavior: "smooth" });
      });
    }

    const btnTriggerCancel = document.getElementById("btn-trigger-cancellation-flow");
    if (btnTriggerCancel) {
      btnTriggerCancel.addEventListener("click", () => {
        window.HardwareManager.vibrate(30);
        document.getElementById("modal-official-contract").style.display = "none";
        document.getElementById("modal-cancellation-request").style.display = "flex";
      });
    }

    const btnDismissCancelModal = document.getElementById("btn-cancel-modal-dismiss");
    if (btnDismissCancelModal) {
      btnDismissCancelModal.addEventListener("click", () => {
        document.getElementById("modal-cancellation-request").style.display = "none";
      });
    }

    const formCancel = document.getElementById("form-submit-cancellation");
    if (formCancel) {
      formCancel.addEventListener("submit", async (e) => {
        e.preventDefault();
        window.HardwareManager.vibrate([60, 40, 60]);
        const reason = document.getElementById("input-cancellation-reason").value.trim();
        const schoolName = currentSelectedPackage ? currentSelectedPackage.schoolName : "Lycée d'Excellence Dakar";

        document.getElementById("modal-cancellation-request").style.display = "none";
        formCancel.reset();

        try {
          await fetch('/api/b2b/cancellation-request', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ schoolName, reason })
          });
        } catch (err) {
        ...
        window.HardwareManager.sendLocalNotification(
          "Termination Request Transmitted",
          `Reason: "${reason}". Live dispatch transmitted to alihandivanli8@gmail.com & +90 541 520 84 14.`
        );

        showToast("Thank you for your feedback! Dispatch transmitted to support center.");
    ...
    // Cost Calculator
    const calcStudentsInput = document.getElementById("calc-students-count");
    const calcPkgSelect = document.getElementById("calc-package-select");
    const calcMonthlyOut = document.getElementById("calc-monthly-out");
    const calcYearlyOut = document.getElementById("calc-yearly-out");

    function runCalculator() {
      if (!calcStudentsInput || !calcPkgSelect) return;
      const count = Number(calcStudentsInput.value) || 500;
      const rate = Number(calcPkgSelect.value) || 2;
      const monthly = count * rate;
      const yearly = monthly * 12;

      calcMonthlyOut.innerText = `$${monthly.toLocaleString()}`;
      calcYearlyOut.innerText = `$${yearly.toLocaleString()}`;
    }

    if (calcStudentsInput && calcPkgSelect) {
      calcStudentsInput.addEventListener("input", runCalculator);
      calcPkgSelect.addEventListener("change", runCalculator);
      runCalculator();
    }
